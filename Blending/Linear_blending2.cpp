...
void linear_blending2()
{
	...
	int overlapWidth = 64;
	cv::rectangle(mask1, cv::Rect(0, 0, (width - overlapWidth) / 2, height), cv::Scalar(1.0f), cv::FILLED);
	cv::rectangle(mask1, cv::Rect((width - overlapWidth) / 2, 0, overlapWidth, height), cv::Scalar(0.5f), cv::FILLED);
	cv::rectangle(mask2, cv::Rect((width + overlapWidth) / 2, 0, (width - overlapWidth) / 2, height), cv::Scalar(1.0f), cv::FILLED);
	cv::rectangle(mask2, cv::Rect((width - overlapWidth) / 2, 0, overlapWidth, height), cv::Scalar(0.5f), cv::FILLED);
